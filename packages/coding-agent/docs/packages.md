...
By default, `install` and `remove` write to global settings (`~/.prime/agent/settings.json`). Use `-l` to write to project settings (`.pi/settings.json`) instead. Project settings can be shared with your team, and Prime Agent installs any missing packages automatically on startup.
...
- Cloned to `~/.prime/agent/git/<host>/<path>` (global) or `.pi/git/<host>/<path>` (project).
...
Use `prime-agent config` to enable or disable extensions, skills, prompt templates, and themes from installed packages and local directories. Works for both global (`~/.prime/agent`) and project (`.pi/`) scopes.
