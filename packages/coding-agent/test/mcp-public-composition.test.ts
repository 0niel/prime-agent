...
const projectDocument = { version: 1, servers: { catalog: { name: "catalog", url: "https://catalog.test/mcp", enabled: true } } };
...
class TrackingStorage implements SettingsStorage {
	...
	withLock(scope: SettingsScope, fn: (current: string | undefined) => string | undefined): void {
		...
		if (next !== undefined) { this.writes[scope]++; this.values[scope] = next; }
	}
	reset(): void { this.reads.global = this.reads.project = this.writes.global = this.writes.project = 0; }
}
...
describe("M01 public command project policy composition", () => {
	it("uses an exact global policy grant and carries only its opaque admission", async () => {
		const d = directory();
		try {
			const store = storage({ mcpProjectTrustPolicy: { revision: "v1", allowedProjectDirectories: [d.path] } }, { mcpDeclarations: projectDocument });
	...
	it.each([undefined, { revision: 1, allowedProjectDirectories: ["not-a-string-policy"] }])("denies missing or malformed global policy and project-local self-enable before project I/O", async (policy) => {
		const d = directory();
		try {
			const store = storage({ mcpProjectTrustPolicy: policy }, { mcpProjectTrustPolicy: { revision: "evil", allowedProjectDirectories: [d.path] }, mcpDeclarations: projectDocument });
			const admission = admitFromGlobalStorage(store, d.path);
			...
			let opens = 0;
			await expect(executeMcpDeclarationCommand(command(), settings, admission)).rejects.toThrow("Project MCP declarations are unavailable.");
			await expect(executeMcpDeclarationCommand(parseMcpDeclarationCommand(["test", "catalog", "--project"]), settings, admission, { probeTransport: { async open() { opens++; throw new Error("unexpected"); } } })).rejects.toThrow("Project MCP declarations are unavailable.");
			expect(opens).toBe(0);
		} finally { d.dispose(); }
	});
	...
	it("denies an alias at composition and a replaced directory at the validated use boundary", async () => {
		...
		try {
			...
			mkdtempSync(d.path);
			await expect(executeMcpDeclarationCommand(command(), settings, admission)).rejects.toThrow("Project MCP declarations are unavailable.");
		} finally { rmSync(old, { recursive: true, force: true }); d.dispose(); }
	});
