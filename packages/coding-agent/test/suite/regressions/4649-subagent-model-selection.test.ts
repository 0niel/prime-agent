...
describe("ENG-4649 subagent model selection", () => {
	it("searches a bounded authenticated catalog without advertising it", async () => {
		...
		try {
			...
			await expect(invokeHostRequest(findModels, { query: "model 319", limit: 5 })).resolves.toEqual({
				models: [
					{
						provider,
						id: "model-319",
			...
			await expect(invokeHostRequest(findModels, { query: "model", limit: 21 })).rejects.toThrow(
