...
describe("goal skill over the kernel host bridge", { tags: ["kernel-heavy"] }, () => {
	...
	it("round-trips goal lifecycle calls through a live kernel", async () => {
		...
		provisioner = new IpythonKernelProvisioner(tempDir, {
			...
			hostHandlers: {
				...
				"goal.pause": async (payload) => {
					...
					return {
						goal: { objective: "ship it", status: "paused", tokens_used: 3, last_reason: payload.reason },
						remaining_tokens: 7,
						completion_budget_report: null,
					};
				},
...
print(_paused["goal"]["status"], _paused["goal"]["last_reason"])
`);
		expect(paused.status).toBe("ok");
		expect(paused.stdout.trim()).toBe("paused waiting for approval");
