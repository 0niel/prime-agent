...
describe("Core MCP declaration snapshot", () => {
	it("creates a frozen declaration-only, code-point-ordered snapshot", () => {
		...
		expect(first).toEqual({
			...
			declarations: { catalog: { name: "catalog", endpoint: "https://catalog.test/mcp", enabled: true, source: "user" } },
		});
	...
	it("does not read project data for omitted, forged, accessor, or copied foreign admissions", () => {
		...
		try {
			...
			const forged = {
				...
				get binding() { fakeValidationCalls++; return {}; },
			};
			for (const projectAdmission of [undefined, {}, forged, foreign]) {
				const snapshot = createMcpRuntimeDeclarationSnapshot({
					...
					readProjectDocument: () => { reads++; return projectDocument; },
				});
			...
			expect(reads).toBe(0);
			expect(resolveProjectMcpDeclarations(projectDocument, forged as never)).toEqual({ document: { version: 1, servers: {} }, effective: false });
	...
	it("reads project data only after the opaque admission validates and makes collisions inert", () => {
		...
		try {
			...
			expect(reads).toBe(1);
			expect(selected.declarations.search).toEqual({ name: "search", endpoint: "https://search.test/mcp", enabled: false, source: "project" });

			const colliding = createMcpRuntimeDeclarationSnapshot({
				userDocument,
				projectAdmission: granted,
				readProjectDocument: () => ({ version: 1, servers: { catalog: { name: "catalog", url: "https://other.test/mcp", enabled: true } } }),
			});
	...
	it("rejects accessor, inherited, and locale-sensitive parser inputs without reading inherited data", () => {
		...
		Object.defineProperty(accessor.servers, "catalog", { enumerable: true, get() { throw new Error("must not get accessor"); } });
		...
		expect(() => createMcpRuntimeDeclarationSnapshot({ userDocument: { version: 1, servers: { "İ": { name: "İ", url: "https://x.test", enabled: true } } } })).toThrow();
	});
});
