...
describe("M01 injected MCP probe", () => {
	...
	it("redacts injected transport failures and closes the session", async () => {
		...
		await expect(runMcpDeclarationProbe(declaration, transport, { trusted: true })).rejects.toThrow("MCP probe failed.");
	...
	it("aborts a hanging injected transport within its bounded timeout", async () => {
		...
		await expect(runMcpDeclarationProbe(declaration, transport, { trusted: true, timeoutMs: 10 })).rejects.toThrow("timed out");
