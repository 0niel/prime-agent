...
- Fixed kernel teardown leaving host-request work running and removed shell-based clipboard command execution ([#915](https://github.com/PrimeIntellect-ai/prime-agent/pull/915)).
...
- Fixed resumed and concurrently repaired sessions losing entries when a crash left an incomplete JSONL tail, including bounded recovery from dead-writer locks and explicit diagnostics for malformed lock metadata ([#928](https://github.com/PrimeIntellect-ai/prime-agent/issues/928)).
- Fixed concurrent settings and credential updates losing unrelated configuration on all platforms, added fsync-backed POSIX write durability and private permissions, and made legacy auth migration commit credentials before best-effort cleanup ([#983](https://github.com/PrimeIntellect-ai/prime-agent/issues/983)).
...
- Fixed Windows daemon startup and worker authentication blocking on slow child startup and synchronous process identity checks ([#1077](https://github.com/PrimeIntellect-ai/prime-agent/issues/1077)).
...
- Fixed compaction recovery amplifying persisted retry debris or sending oversized summary requests ([#900](https://github.com/PrimeIntellect-ai/prime-agent/issues/900)).
- Added model-driven goal pause and resume transitions so externally blocked goals stop auto-continuing ([#986](https://github.com/PrimeIntellect-ai/prime-agent/issues/986)).
