...
export class DaemonSupervisor {
	...
	private async recoverUncertainWorkerOperations(worker: ResidentWorker, killWorkerProcess: boolean): Promise<void> {
		await this.assertRecoveryAllowed();
		const descriptor = worker.descriptor;
	...
	private signalCurrentWorker(
		...
		// A direct ChildProcess object is an unforgeable handle to this
		// supervisor's just-spawned child. Validate only that it is still that
		// descriptor pid and live, then signal the handle itself. Do not require
		// a persisted start-id or mutable registration tuple on launch rollback.
		if (directChild) {
			if (directChild.pid !== descriptor.pid || directChild.exitCode !== null || directChild.signalCode !== null) {
				return false;
			}
		...
		// Descriptor-only signalling must fail closed: a legacy descriptor has
		// no generation identity with which to distinguish a recycled pid.
		if (
	...
	private async stopWorkerUntracked(
		...
		const assertStopStillApplies = () => {
			if (
				...
				throw new Error(`Session worker ${entryDescriptor.workerId} was relaunched during stop`);
			}
		};
