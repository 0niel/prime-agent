...
export interface ProcessIdentityProbe {
	...
	getStartId: (pid: number) => string | undefined;
}

/** @internal Exported for deterministic PID-reuse regression coverage. */
export function processIdentityMatches(
	identity: ProcessIdentity,
	allowUnknownStartId: boolean,
	probe: ProcessIdentityProbe = { isAlive: isProcessAlive, getStartId: getProcessStartId },
): boolean {
	if (!probe.isAlive(identity.pid)) {
		return false;
	}
	if (!identity.processStartId) {
		return true;
	}
	// Do not cache this lookup: Windows can recycle a PID while a prior start ID
	// is still cached, causing a stale supervisor identity to appear current.
	const observed = probe.getStartId(identity.pid);
	return observed === identity.processStartId || (allowUnknownStartId && observed === undefined);
}

function isProcessIdentityAlive(identity: ProcessIdentity): boolean {
	return processIdentityMatches(identity, true);
}

function matchesExactProcessIdentity(identity: ProcessIdentity): boolean {
	return processIdentityMatches(identity, false);
}
