    (function() {
      ...
      function getTreeNodeDisplayHtml(entry, label) {
        ...
        switch (entry.type) {
          ...
          case 'compaction':
            return labelHtml + `<span class="tree-compaction">[compaction: ${Math.round(entry.tokensBefore/1000)}k${entry.tokensAfter === undefined ? "" : ` → ${Math.round(entry.tokensAfter/1000)}k`} tokens]</span>`;
          case 'branch_summary': {
      ...
      function renderEntry(entry) {
        ...
        if (entry.type === 'compaction') {
          return `<div class="compaction" id="${entryDomId}" onclick="if(window.getSelection().toString())return;this.classList.toggle('expanded')">
            ...
            <div class="compaction-collapsed">Compacted ${entry.tokensAfter === undefined ? `from ${entry.tokensBefore.toLocaleString()}` : `${entry.tokensBefore.toLocaleString()} → ${entry.tokensAfter.toLocaleString()}`} tokens</div>
            <div class="compaction-content"><strong>Compacted ${entry.tokensAfter === undefined ? `from ${entry.tokensBefore.toLocaleString()}` : `${entry.tokensBefore.toLocaleString()} → ${entry.tokensAfter.toLocaleString()}`} tokens</strong>\n\n${escapeHtml(entry.summary)}</div>
          </div>`;
        }
